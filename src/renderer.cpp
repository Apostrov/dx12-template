...
void Renderer::LoadAssets()
{
	...
	ThrowIfFailed(D3D12SerializeRootSignature(&rootSignatureDescriptor, D3D_ROOT_SIGNATURE_VERSION_1,
		&signature, &error));
	...
	ColorVertex triangleVerteces[] = {
		{{0.f * aspect_ratio, 0.25f, 0.f}, {1.f, 0.f, 0.f, 1.f}},
		{{0.25f * aspect_ratio, -0.25f, 0.f}, {0.f, 1.f, 0.f, 1.f}},
		{{-0.25f * aspect_ratio, -0.25f, 0.f}, {0.f, 0.f, 1.f, 1.f}}
	};

	const UINT vertexBufferSize = sizeof(triangleVerteces);
	...
	memcpy(vertexDataBegin, triangleVerteces, sizeof(triangleVerteces));
